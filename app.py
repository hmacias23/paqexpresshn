...
def contact_form():
    if request.method == 'POST':
        ...
        try:
            ...
            subject = f'Nueva incidencia de: {name}'
            body = f'Nombre: {name}\ntelefono: {telefono}\n\nRespuestas:\n\n' \
            ...
            # Envía el correo electrónico solo si la respuesta a la pregunta es "Si"
            if negative_experience.lower() == 'si':
                server.sendmail(EMAIL_ADDRESS, 'support@paqexpress.freshdesk.com', msg.encode('utf-8'))  # Codificamos el mensaje como UTF-8

            flash('Encuesta enviada con éxito!', 'success')
